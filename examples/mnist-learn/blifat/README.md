...
1. Constants required for the input data processing, such as the number of steps to process an image and the number of classes in the dataset, are defined.

1. The `main` function is implemented that processes the images and their labels, splits the dataset into training and test sets, trains the neural network on the training set and runs inference on the test set.
   ...
   1. Reads the paths to images and labels files and the path to log folder.
   1. Defines the `std::filesystem::path` object to store path to the backend.
   1. Defines the `knp::framework::data_processing::classification::images::Dataset` object to store the MNIST dataset.
   1. Defines the `std::ifstream` objects to read images and labels from the specified paths.
   1. Processes images and labels files from the `std::ifstream` objects using the `process_labels_and_images` function.
   1. Splits the MNIST dataset into training and test sets using the `split` function.
   1. Trains a neural network model using the `train_mnist_network` function and stores the trained model in an `AnnotatedNetwork` object.
   1. Runs inference on the test set using the `run_mnist_inference` function, and stores output spikes in a vector.
   1. Evaluates the results the `process_inference_results` function.
   1. Writes evaluation results to console and log file using the `write_inference_results_to_stream_as_csv` function.
